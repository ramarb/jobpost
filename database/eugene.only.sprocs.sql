...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_company_create` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
CREATE  PROCEDURE `sp_company_create`(
	...
	IN p_users_id INT,
	IN p_name VARCHAR(255) CHARSET utf8,
	IN p_description TEXT CHARSET utf8,
	IN p_job_categories_id INT,
	IN p_cities_id INT,
	IN p_address VARCHAR(255) CHARSET utf8,
    IN p_contact_number VARCHAR(255) CHARSET utf8,
	OUT alert_message TEXT CHARSET utf8,
	OUT return_id INT
)
BEGIN
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
	BEGIN
		SET alert_message = "SQL EXCEPTION ERROR; Procedure Name : sp_company_create";
	END;

	#DECLARE EXIT HANDLER FOR SQLWARNING
	#BEGIN
		#SET alert_message = "SQL WARNING; Procedure Name : sp_company_create";
	#END;

	DECLARE CONTINUE HANDLER FOR NOT FOUND SET alert_message = "SQL CONTINUE HANDLER; Procedure Name : sp_company_create";
   
	SET @companies_id = NULL;	

	SELECT `id` INTO @companies_id FROM `companies` WHERE `users_id` = p_users_id LIMIT 1;
    
    IF p_cities_id = '' THEN
		SET p_cities_id = NULL;
	END IF;
	
	IF p_job_categories_id = '' THEN
		SET p_job_categories_id = NULL;
	END IF;

	IF @companies_id IS NULL THEN
		
        INSERT INTO `companies`(`users_id`,`name`,`description`,`job_categories_id`,`cities_id`,`address`,`contact_number`)
        VALUES(p_users_id, p_name, p_description, p_job_categories_id, p_cities_id, p_address, p_contact_number);

		SET @companies_id = LAST_INSERT_ID();

		SET alert_message = "Success";

		SET return_id = @companies_id;
	...
	ELSE
		...
		UPDATE `companies` SET
			...
			`job_categories_id` = p_job_categories_id,
			`cities_id` = p_cities_id,
			`address` = p_address,
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_education_create_update` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,STRICT_ALL_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ALLOW_INVALID_DATES,ERROR_FOR_DIVISION_BY_ZERO,TRADITIONAL,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE  PROCEDURE `sp_education_create_update`(
	IN p_educations_id INT,
	IN p_users_id INT,
	IN p_education_types_id INT,
    IN p_course VARCHAR(255) CHARSET utf8,
    IN p_school VARCHAR(255) CHARSET utf8,
    IN p_date_from DATE,
    IN p_date_to DATE,
	IN p_cities_id INT,
	IN p_address VARCHAR(255) CHARSET utf8,
    IN p_achievements TEXT CHARSET utf8,
	OUT alert_message TEXT CHARSET utf8,
	OUT return_id INT
)
BEGIN
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
	BEGIN
		SET alert_message = "SQL EXCEPTION ERROR; Procedure Name : sp_education_create_update";
	END;

	#DECLARE EXIT HANDLER FOR SQLWARNING
	#BEGIN
		#SET alert_message = "SQL WARNING; Procedure Name : sp_education_create_update";
	#END;

	DECLARE CONTINUE HANDLER FOR NOT FOUND SET alert_message = "SQL CONTINUE HANDLER; Procedure Name : sp_education_create_update";
   
	IF p_cities_id = '' THEN
		SET p_cities_id = NULL;
	END IF;
	
	IF p_education_types_id = '' THEN
		SET p_education_types_id = NULL;
	END IF;

	IF p_educations_id = '0' THEN
		
        INSERT INTO `educations`(`users_id`,`education_types_id`,`course`,`school`,`date_from`,`date_to`,`cities_id`,`address`,`achievements`)
        VALUES(p_users_id, p_education_types_id, p_course, p_school, p_date_from, p_date_to, p_cities_id, p_address, p_achievements);

		SET @educations_id = LAST_INSERT_ID();

		SET alert_message = "Success";

		SET return_id = @educations_id;

	ELSE

		UPDATE `educations` SET
			`users_id` = p_users_id,
            `education_types_id` = p_education_types_id,
            `course` = p_course,
            `school` = p_school,
        ...
        WHERE `id` = p_educations_id;

		SET alert_message = "Success";

		SET return_id = p_educations_id;	
...
CREATE  PROCEDURE `sp_insert`(
	IN tbl VARCHAR(255) CHARSET utf8,
	IN tbl_fields TEXT CHARSET utf8,
	IN tbl_fields_value TEXT CHARSET utf8,
	OUT return_id INT,
...
CREATE  PROCEDURE `sp_user_create`(
	...
	IN p_role_name VARCHAR(255) CHARSET utf8,
	IN p_first_name VARCHAR(255) CHARSET utf8,
	IN p_last_name VARCHAR(255) CHARSET utf8,
	IN p_email VARCHAR(255) CHARSET utf8,
	IN p_password VARCHAR(255) CHARSET utf8,
	IN p_user_state VARCHAR(255) CHARSET utf8,
	OUT alert_message TEXT CHARSET utf8,
	OUT return_id INT
)
BEGIN
...
CREATE  PROCEDURE `sp_user_files_create`(
	IN p_users_id INT,
	IN p_location TEXT CHARSET utf8,
	IN p_name VARCHAR(250) CHARSET utf8,
	IN p_size VARCHAR(45) CHARSET utf8,
	IN p_type VARCHAR(45) CHARSET utf8,
	IN p_user_file_type VARCHAR(50) CHARSET utf8,
	IN p_user_file_state VARCHAR(50) CHARSET utf8,
	OUT alert_message TEXT CHARSET utf8,
	OUT return_id INT
)
BEGIN
...
CREATE  PROCEDURE `sp_vacancy_create`(
	...
	IN p_job_categories_id INT,
	IN p_cities_id INT,
	IN p_company VARCHAR(255)  CHARSET utf8,
	IN p_title VARCHAR(255)  CHARSET utf8,
	IN p_description TEXT CHARSET utf8,
	IN p_salary VARCHAR(255)  CHARSET utf8,
	IN p_address VARCHAR(255) CHARSET utf8,
	OUT alert_message TEXT CHARSET utf8,
	OUT return_id INT
)
...
-- Dump completed on 2016-11-14 16:55:21
